...
static int _blkid_exit(struct module *module, struct sid_ucmd_mod_ctx *cmd_mod)
{
	 ...
	 * 	TODO: Do not unreserve KVs in worker if we have modules preloaded in master process.
		for (i = _UDEV_KEY_START; i <= _UDEV_KEY_END; i++) {
			if (sid_ucmd_mod_unreserve_kv(module, cmd_mod, KV_NS_UDEV, keys[i]) < 0) {
		...
		for (i = _DEVICE_KEY_START; i <= _DEVICE_KEY_END; i++) {
			if (sid_ucmd_mod_unreserve_kv(module, cmd_mod, KV_NS_DEVICE, keys[i]) < 0) {
...
static void _add_property(struct sid_ucmd_ctx *cmd, const char *name, const char *value)
{
	...
	if (!strcmp(name, "TYPE")) {
		len = strlen(value);
		sid_ucmd_set_kv(cmd, KV_NS_UDEV, keys[ID_FS_TYPE], value, len + 1, KV_MOD_PROTECTED);
		...
		sid_ucmd_set_kv(cmd, KV_NS_UDEV, keys[ID_FS_VERSION], value, strlen(value) + 1, KV_MOD_PROTECTED);
	} else if (!strcmp(name, "UUID")) {
		blkid_safe_string(value, s, sizeof(s));
		sid_ucmd_set_kv(cmd, KV_NS_UDEV, keys[ID_FS_UUID], value, strlen(value) + 1, KV_MOD_PROTECTED);
		blkid_encode_string(value, s, sizeof(s));
		sid_ucmd_set_kv(cmd, KV_NS_UDEV, keys[ID_FS_UUID_ENC], s, strlen(s) + 1, KV_MOD_PROTECTED);
	} else if (!strcmp(name, "UUID_SUB")) {
		blkid_safe_string(value, s, sizeof(s));
		sid_ucmd_set_kv(cmd, KV_NS_UDEV, keys[ID_FS_UUID_SUB], value, strlen(value) + 1, KV_MOD_PROTECTED);
		blkid_encode_string(value, s, sizeof(s));
		sid_ucmd_set_kv(cmd, KV_NS_UDEV, keys[ID_FS_UUID_SUB_ENC], s, strlen(s) + 1, KV_MOD_PROTECTED);
	} else if (!strcmp(name, "LABEL")) {
		blkid_safe_string(value, s, sizeof(s));
		sid_ucmd_set_kv(cmd, KV_NS_UDEV, keys[ID_FS_LABEL], s, strlen(s) + 1, KV_MOD_PROTECTED);
		blkid_encode_string(value, s, sizeof(s));
		sid_ucmd_set_kv(cmd, KV_NS_UDEV, keys[ID_FS_LABEL_ENC], s, strlen(s) + 1, KV_MOD_PROTECTED);
	} else if (!strcmp(name, "PTTYPE")) {
		sid_ucmd_set_kv(cmd, KV_NS_UDEV, keys[ID_PART_TABLE_TYPE], value, strlen(value) + 1, KV_MOD_PROTECTED);
	} else if (!strcmp(name, "PTUUID")) {
		sid_ucmd_set_kv(cmd, KV_NS_UDEV, keys[ID_PART_TABLE_UUID], value, strlen(value) + 1, KV_MOD_PROTECTED);
	} else if (!strcmp(name, "PART_ENTRY_NAME")) {
		blkid_encode_string(value, s, sizeof(s));
		sid_ucmd_set_kv(cmd, KV_NS_UDEV, keys[ID_PART_ENTRY_NAME], s, strlen(s) + 1, KV_MOD_PROTECTED);
	} else if (!strcmp(name, "PART_ENTRY_TYPE")) {
		blkid_encode_string(value, s, sizeof(s));
		sid_ucmd_set_kv(cmd, KV_NS_UDEV, keys[ID_PART_ENTRY_TYPE], s, strlen(s) + 1, KV_MOD_PROTECTED);
	} else if (!strncmp(name, "PART_ENTRY_", strlen("PART_ENTRY_"))) {
		snprintf(s, sizeof(s), "ID_%s",	name);
		sid_ucmd_set_kv(cmd, KV_NS_UDEV, s, value, strlen(value) + 1, KV_MOD_PROTECTED);
	} else if (!strcmp(name, "SYSTEM_ID")) {
		blkid_encode_string(value, s, sizeof(s));
		sid_ucmd_set_kv(cmd, KV_NS_UDEV, keys[ID_FS_SYSTEM_ID], s, strlen(s) + 1, KV_MOD_PROTECTED);
	} else if (!strcmp(name, "PUBLISHER_ID")) {
		blkid_encode_string(value, s, sizeof(s));
		sid_ucmd_set_kv(cmd, KV_NS_UDEV, keys[ID_FS_PUBLISHER_ID], s, strlen(s) + 1, KV_MOD_PROTECTED);
	} else if (!strcmp(name, "APPLICATION_ID")) {
		blkid_encode_string(value, s, sizeof(s));
		sid_ucmd_set_kv(cmd, KV_NS_UDEV, keys[ID_FS_APPLICATION_ID], s, strlen(s) + 1, KV_MOD_PROTECTED);
	} else if (!strcmp(name, "BOOT_SYSTEM_ID")) {
		blkid_encode_string(value, s, sizeof(s));
		sid_ucmd_set_kv(cmd, KV_NS_UDEV, keys[ID_FS_BOOT_SYSTEM_ID], s, strlen(s) + 1, KV_MOD_PROTECTED);
	}
}
