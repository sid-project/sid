...
int kv_store_unset_value_ht(sid_resource_t *kv_store_res, const char *key, kv_store_update_fn_t kv_unset_fn, void *kv_unset_fn_arg)
{
	struct kv_store *           kv_store = sid_resource_get_data(kv_store_res);
	struct kv_store_value *     found;
	struct kv_store_update_spec update_spec = {0};
...
kv_store_iter_t *kv_store_iter_create_ht(sid_resource_t *kv_store_res)
{
	...
	if (!(iter = malloc(sizeof(*iter))))
		return NULL;
	iter->backend = KV_STORE_BACKEND_HASH;
...
{
	if (return_key != NULL)
		*return_key = kv_store_iter_current_key_ht(iter);
...
int _init_kv_store_ht(sid_resource_t *kv_store_res, const void *kickstart_data, void **data)
{
	...
	kv_store->backend = KV_STORE_BACKEND_HASH;
...
int _destroy_kv_store_ht(sid_resource_t *kv_store_res)
{
	struct kv_store *kv_store = sid_resource_get_data(kv_store_res);
...
const sid_resource_type_t sid_resource_type_kv_store_ht = {
	.name    = KV_STORE_NAME,
	.init    = _init_kv_store_ht,
	.destroy = _destroy_kv_store_ht,
};
