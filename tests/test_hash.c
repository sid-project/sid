...
static void test_hash_add()
{
	...
	for (int i = 0; i < KEY_COUNT; i++)
		assert_int_equal(hash_add(t, test_array[i], strlen(test_array[i]) + 1, test_array[i], strlen(test_array[i]) + 1),
		                 0);

	assert_int_equal(hash_get_entry_count(t), KEY_COUNT);

	hash_wipe(t);
	assert_int_equal(hash_get_entry_count(t), 0);
	...
	for (int i = 0; i < KEY_COUNT; i++)
		assert_int_equal(hash_add_allow_multiple(t,
		                                         test_array[i],
	...
	assert_int_equal(hash_get_entry_count(t), KEY_COUNT * 2);
